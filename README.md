...
- **Valid JSON:** Double-check that your addition does not break the overall JSON syntax (e.g., watch out for missing commas between objects or unclosed brackets).

### 2. Include Required Fields

Your member object must include the following valid properties:
...
- **Consent:** By including your email, you are opting in to receive an automated notification whenever a new member is added to the webring. The purpose of this is to allow established members to visit new portfolios and provide constructive feedback.
- **Opting Out:** If you wish to stop receiving these emails in the future, you must open a new Pull Request to remove the `email` property from your object, or contact a repository maintainer to do it for you.

### 6. Add the Widget to Your Site

For the webring to function, you must link back to it on your own website. You need to provide "Next" and "Previous" navigation links, and ideally a link to the webring landing page itself.

Here is an example of what your HTML `href` tags should look like (make sure to replace the `?from=` URL with your actual website URL):
